...
/**
 * Generated class for the ApovpckPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-apovpck',
  templateUrl: 'apovpck.html',
})
export class ApovpckPage {
...
vim:boolean;
  ...
  constructor(public navCtrl: NavController,public alert:AlertController,public tos:ToastController ,public http:Http,public httc: HttpClient, public navParams: NavParams) {
    this.vdetail = false;
    this.getdata();
    this.getarea();
    this.vim=false;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad ApovpckPage');
  }
  
  getdata() {
    let prm={fn:'adminapprove'}
...
console.log("city"+this.city);*/
  ...
  reject() {
console.log(this.da[this.s].pckid);
let prm={fn:'rejectpackag',id:this.da[this.s].pckid};
this.httc.post("http://localhost/hmsproj/index.php",prm).subscribe(data => {
  console.log(data);
  let al=this.alert.create({
    
     subTitle:'Package Rejected',
     buttons:[
      {
        text:'ok',
