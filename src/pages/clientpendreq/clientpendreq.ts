...
/**
 * Generated class for the ClientpendreqPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-clientpendreq',
  templateUrl: 'clientpendreq.html',
})
export class ClientpendreqPage {
  obj: any;
  o2: any;
  da: any;
  s: any;
  ...
  ia: any = [];
  bl:any;
  constructor(public navCtrl: NavController,public alert:AlertController, public tos: ToastController, public sha: ShareserviceProvider, public httc: HttpClient, public navParams: NavParams) {
    this.getappreqdata();
    this.vdetail = false;
    this.vim = false;
    this.bl="";
  ...
  getappreqdata() {
    ...
    let prm = { fn: 'clientpenreq', uid: this.sha.getuserid() }
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad clienaproved');
  }
  ...
  delreq(i) {
    console.log(i);
let prm={'fn':'clidelreq',reqid:i};
this.httc.post("http://192.168.43.96/hmsproj/index.php",prm).subscribe(data=>{
  console.log(data);
  if((JSON.parse((JSON.stringify(data))))=="s"){
    let al=this.alert.create({
      title:'Cancle Request',
      subTitle:'Request cancled',
      buttons:[{
        text:'ok',
        role:'ok'
      }
  ...
  cancle() {
    //console.log(this.da[this.fmid].reqid);
      this.delreq(this.da[this.fmid].reqid);
  }
  view(i) {
    var id = i;
    this.vdetail = true;
